...
int CALLBACK MyCallback (WORD wEvent, int nReference, DWORD dwParam, DWORD dwUser)
{
    ...
    //printf("MyCallback is called,event is %d\r\n",wEvent);
    ...
    if(nMaxCh >= 0)
    {
        for(int ch=0, i=1; ch<nMaxCh; ch++)
        {
            ...
            case 2:
                ...
                GetPrivateProfileString(lpAppName, KeyName, "XXXXXXXX", (LPSTR)ChInfo[ch].CUserNum.c_str(), 16, szPhoneNumConfig);
                ...
                i++;
                //nPhoneNum++;
                UserIdleQueue.push(ch);   //初始化空闲坐席队列，add by gsc, 2012-5-13 22:08:38
...
LVS_UINT32 LVS_PbxMsgProc(LVS_VOID *pMsg)
{
    ...
    printf("LVS_PbxMsgProc, event id = %d\r\n",pstReqMsg->ulMsgId);
...
LVS_VOID UserProc(LVS_INT32 event, LVS_INT32 wParam, LVS_INT32 lParam)
{
	...
	case E_CHG_HookState:		//station hook change event
		...
		if(nHook == 0)			//station channel go on-hook 
		{
            ...
            //ShowErrMsg("Insert Into USER_IDLE Queue!!!!", " ***************1111111111111111111*************", "");
            //UserIdleQueue.push(nCh);
			...
			if(dwDtmfLen == MAX_USER_DTMFLEN)
			{
				...
				for(int i=0; i<nMaxCh; i++)
				{
					...
					if ( (ChInfo[i].nChType == 2)
						&& (strcmp(ChInfo[nCh].szPhoneNumBuf, ChInfo[i].CUserNum.c_str()) == 0 ) && i != nCh)
					{
	...
	case E_US_SEIZURE:
		...
		{	
			...
			{
				strcpy(ChInfo[nCh].szCallerId, ChInfo[ChInfo[nCh].nLinkToCh].CUserNum.c_str());
			}
		}
		else
		{
	...
	case E_US_RELEASE:
		...
		ChInfo[nCh].nStatus = USER_IDLE;
        //UserIdleQueue.push(nCh);
        //ShowErrMsg("Insert Into USER_IDLE Queue!!!!", " **************2222222222222222222*************", "");
        ...
        case TRUNK_IDLE:
            ...
            {       
                ...
                */
                ...
                //ChInfo[nCh].nStatus = TRUNK_WAIT_SELECTION;  修改 by gsc，2012-5-12 23:57:37
                
                //ChInfo[nCh].nLinkToCh = GetAnIdleUser();   修改 by gsc, 2012年5月19日22:24:43
                ChInfo[nCh].nLinkToCh = 0;
    ...
    case E_CHG_RcvDTMF:
        ...
        case TRUNK_WAIT_SELECTION:
            ...
            if(dwDtmfLen == MAX_USER_DTMFLEN)
            {
                ...
                ChInfo[nCh].szPhoneNumBuf[MAX_USER_DTMFLEN] = '\0';
				printf("[1]******** Here is in TRUNK_WAIT_SELECTION,  dwDtmfLen= %d ************\r\n",dwDtmfLen);
              /*for(int i=0; i<nMaxCh; i++)
                {
                    if( (ChInfo[i].nChType == 2)
                        && (strcmp(ChInfo[nCh].szPhoneNumBuf, ChInfo[i].CUserNum.c_str()) == 0) && i != nCh )
            ...
            {
                ChInfo[nCh].szPhoneNumBuf[dwDtmfLen-1] = dwDtmfNum;
				printf("[2]******** Here is in TRUNK_WAIT_SELECTION, dwDtmfLen= %d and dwDtmfNum = %d ************\r\n",dwDtmfLen, dwDtmfNum);
                if( (ChInfo[nCh].nTimer = SsmStartTimer(8000, 1)) == -1)
                {
...
LVS_VOID ShowErrMsg(LPCTSTR lpszEventText, LPCTSTR lpszStatusText, LPCTSTR lpszFunctionText)
{
    ...
    if(NULL != lpszFunctionText)
    {
        cout<< lpszFunctionText<<"."<<endl;
    }
}
...
LVS_INT32 GetAnIdleUser()
{
    ...
    while(1)
    {
        ChId = UserIdleQueue.pop();
        if(TRUE == UserIdleQueue.empty())
        {
            ShowErrMsg("GET_IDLE_USER", "THE_QUEUE_IS_EMPTY", "THERE_IS_NO_ANY_IDLE_USER_CHANNEL!!!");
            break;
        }
        if(USER_IDLE == ChInfo[ChId].nStatus)
            return ChId;
    }

    for(LVS_INT32 i=0; i<nMaxCh; i++)
    {
        if(ChInfo[i].nChType == 2)
        {
            ChId = i;  //如果为空折返回0，即为第一个坐席通道
        }               
    }
